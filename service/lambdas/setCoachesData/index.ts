...
export default ({ props, construct }: LambdaProps) => {
    ...
    const setCoachesData = new NodejsFunction(construct, nameAndID, {
    ...
    new Rule(construct, `${nameAndID}-data-update`, {
        ruleName: `${deploymentType}-coach-data-update`,
        description: "Regular update to the coaches.json static data file",
        targets: [new LambdaFunction(setCoachesData)],
        schedule: Schedule.rate(Duration.days(7)),
    });

    return setCoachesData;
};
