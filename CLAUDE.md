...
dropped). Unlike historicalLogos, the images are *not* checked into the repo —
this corpus is heavier and the artwork itself (a watermarked illustration, not
a transparent cutout) has nothing to key out — so each row's `jersey` field
points at a CloudFront URL instead of a local path. That CDN fronts the
