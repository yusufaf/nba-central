...
both the repo and Vite's import graph. (The copies still under
`apps/web/public/logos/historical/` are the pre-CDN originals, kept only until
the CDN path is verified in production — issue #66.) There is no Lambda for
...
legacy checked-in PNGs without re-crawling, and is idempotent.
